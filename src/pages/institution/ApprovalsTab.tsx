...
export default function ApprovalsTab() {
  ...
  const [modal, setModal] = useState<typeof APPROVALS[0] | null>(null);

  const filtered = APPROVALS.filter((a) =>
    tab === "All" ? true :
    tab === "Pending" ? (a.status === "Pending" || a.status === "Under Review") :
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle={`${APPROVALS.filter((a) => a.status === "Pending" || a.status === "Under Review").length} pending actions require attention`}
        actions={<Btn variant="secondary" size="sm">📊 Report</Btn>}
      />

      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Pending", count: APPROVALS.filter((a) => a.status === "Pending").length, color: "bg-blue-50 text-[#0C447C] border-blue-100" },
          { label: "Under Review", count: APPROVALS.filter((a) => a.status === "Under Review").length, color: "bg-purple-50 text-purple-700 border-purple-100" },
          { label: "Approved", count: APPROVALS.filter((a) => a.status === "Approved").length, color: "bg-emerald-50 text-emerald-700 border-emerald-100" },
          { label: "Rejected", count: APPROVALS.filter((a) => a.status === "Rejected").length, color: "bg-red-50 text-red-700 border-red-100" },
        ].map((s) => (
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 text-xs text-slate-400 mb-3">
                  ...
                  <span>📍 Level: <strong className="text-slate-600">{a.level}</strong></span>
                  <span>📅 Due: {a.due}</span>
                </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {(a.status === "Pending" || a.status === "Under Review") && (
                  <div className="flex gap-1">
                    <button onClick={() => setModal(a)} className="px-3 py-1.5 bg-emerald-600 text-white text-xs rounded-lg hover:bg-emerald-700 font-medium">✓ Approve</button>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title="Approve Request" size="sm">
        {modal && (
          <div className="p-5">
            ...
            <div className="text-xs text-slate-400 mb-4">{modal.type} · {modal.requestedBy}</div>
            <FormField label="Approval Comment">
              <textarea className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" rows={3} placeholder="Add your comments (optional)…" />
            </FormField>
            <div className="flex justify-end gap-2 mt-4">
              <Btn variant="secondary" onClick={() => setModal(null)}>Cancel</Btn>
              <Btn variant="success" onClick={() => setModal(null)}>✓ Confirm Approval</Btn>
            </div>
          </div>
        )}
      </Modal>
