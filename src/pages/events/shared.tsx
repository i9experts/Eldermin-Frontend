...
export const Modal = ({ title, onClose, children, footer, wide }: { title: string; onClose: () => void; children: React.ReactNode; footer?: React.ReactNode; wide?: boolean }) => (
...
export const FormField = ({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) => (
  <div>
    <label className="block text-xs font-semibold text-slate-600 mb-1.5">{label}{required && <span className="text-red-500 ml-0.5">*</span>}</label>
    {children}
  </div>
);
...
export const FInput = (props: React.InputHTMLAttributes<HTMLInputElement>) => <input {...props} className={`${inputCls} ${props.className ?? ''}`} />;
export const FTextarea = (props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) => <textarea {...props} className={`${inputCls} resize-vertical ${props.className ?? ''}`} />;
export const FSelect = ({ children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement>) => <select {...props} className={`${inputCls} bg-white ${props.className ?? ''}`}>{children}</select>;
...
export const EmptyState = ({ icon = '🎟️', title, action }: { icon?: string; title: string; action?: React.ReactNode }) => (
